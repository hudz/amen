...
nc.project = {
		...
		ejkm : function(options, container, callback){
			...
			try{
				...
				menuEJKM(); // get menu ejkm
				tipPeranan();	// cluetip
				////tabPeranan();	// get tab peranan
				features();
				breadcrumbEJKM();
				scrollBreadcrumb();
				dropdownUser(); // menu personal
				fadeInContent();
				////taskPending();
				...
				automateCalendar();
				////replaceTiming();
				blankInput();	// to nullize all input ??
				replaceImageButton();
				fixFlushContent();
				////changeURLUIController();
				jQuery(document).on('applyWidgets',function(){highlightRow();});
				highlightRow();	// highlight row
				pageInfo();		// rightCol page information
				formChangeListener();
				windowResize();
				preventLogout();
				...
				checkPasswordRetention();
				
				// EJKM new Request 250414
				sortHeaderBil();
				//removeHeaderBil();
				sortDropAscending();
			}catch(e){
